...
class Critic(object):
    ...
    def learn(self, s, r, s_):
        v_ = self.model(np.array([s_])) #把新状态放入模型中得到一个分数
        with tf.GradientTape() as tape:
            v = self.model(np.array([s]))#把旧状态放入模型中得到一个分数
            ## [敲黑板]计算TD-error
    ...
    if args.train:
        ...
        for i_episode in range(MAX_EPISODE):
            ...
            while True:
                ...
                a = actor.choose_action(s) #根据当前状态最佳动作a

                s_new, r, done, info = env.step(a)#做完a后，新状态s_new，获得reward ，游戏是否结束
